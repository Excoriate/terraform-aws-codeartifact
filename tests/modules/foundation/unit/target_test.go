...
// TestFoundationTargetDeploymentWithCustomConfig tests deployment with custom configuration
func TestFoundationTargetDeploymentWithCustomConfig(t *testing.T) {
	// Skip this test if running in short mode
	if testing.Short() {
		t.Skip("Skipping TestFoundationTargetDeploymentWithCustomConfig in short mode")
	}
	...
	// Detailed logging of module directory
	t.Logf("🔍 Terraform Target Directory (Custom Config): %s", terraformOptions.TerraformDir)
	...
	// Validate outputs with custom configuration
	kmsKeyId := terraform.Output(t, terraformOptions, "kms_key_id")
	require.NotEmpty(t, kmsKeyId, "KMS Key ID should not be empty")
	...
	t.Logf("💾 Resources created with custom config: KMS Key ID: %s, S3 Bucket: %s, CloudWatch Log Group: %s",
		kmsKeyId, s3BucketId, cwLogGroupArn)
}
